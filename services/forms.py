...
class PackageForm(CustomFieldFormMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'description': forms.Textarea(attrs={'rows': 3, 'placeholder': 'Coverage notes, installation requirements, or customer-facing details'}),
        }
        help_texts = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        apply_tailwind(self)
        for field_name in ('monthly_fee', 'setup_fee'):
            widget = self.fields[field_name].widget
            widget.attrs['class'] = f"{widget.attrs.get('class', '')} pl-12 text-right tabular-nums".strip()
